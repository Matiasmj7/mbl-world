...
let trabajando = false; // Control de anti-spam para el botón trabajar
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // 1. ESTADO DE SESIÓN Y CARGA DE VARIABLES ECONÓMICAS
        ...
        if(user) {
            ...
            db.collection('ninjas').doc(user.uid).onSnapshot(doc => {
                if (doc.exists) {
                    ...
                    renderizarTienda(); // Recargar tienda para ver botones "Equipar" actualizados
            ...
            document.getElementById('mi-ryos-bingo').innerHTML = `<i class="fas fa-coins"></i> 0 Ryos`;
            document.getElementById('tienda-mis-ryos').innerHTML = `<i class="fas fa-coins"></i> 0 Ryos`;
    ...
    if(formNick) {
        formNick.addEventListener('submit', (e) => {
            ...
            db.collection('ninjas').doc(currentUserId).set({
                nick: nuevoNick, xp: 0, ryos: 100, rango: "Guerrero", clan: "", inventario: [], equipado: {borde: '', colorChat: '', pin: ''}, email_oculto: auth.currentUser.email, fecha_registro: firebase.firestore.FieldValue.serverTimestamp()
            }).then(() => {
                alert("¡Identidad Ninja creada! Se te han otorgado 100 Ryos de bienvenida.");
    ...
    // BYAKUGAN Y RANKINGS (SE MANTIENEN IGUAL)
    ...
    db.collection('ninjas').orderBy('xp', 'desc').limit(30).onSnapshot(snap => {
        ...
        if(rankingContainer) {
            ...
            snap.forEach(doc => {
                ...
                // Aplica el color equipado si lo tiene, si no azul
                let colorNombre = 'var(--blue)';
                if(data.equipado && data.equipado.colorChat) {
                    const itemColor = CATALOGO_TIENDA.find(i => i.id === data.equipado.colorChat);
                    if(itemColor) colorNombre = itemColor.estilo; 
                }
                ...
                rankingContainer.innerHTML += `
                    <div style="display: flex; justify-content: space-between; align-items: center; background: #000; padding: 10px 15px; border-radius: 5px; border-left: 3px solid ${colorBorde}; cursor: pointer; transition: 0.3s;" onclick="abrirPerfil('${data.nick}')" onmouseover="this.style.background='#111'" onmouseout="this.style.background='#000'">
                        <div><strong>${puesto}. <span style="${colorNombre.includes('color:') ? colorNombre : 'color:'+colorNombre}">${data.nick}</span></strong></div>
                        <div style="font-size: 0.8rem; color: #aaa; text-align: right;">${data.xp || 0} XP<br><span style="color: var(--green);"><i class="fas fa-coins"></i> ${data.ryos || 0}</span></div>
                    </div>
                `;
                puesto++;
            });
    ...
    // TABERNA CON COLORES PERSONALIZADOS
    ...
    if(chatContainer) {
        db.collection('taberna').orderBy('timestamp').onSnapshot(snap => {
            ...
            snap.forEach(doc => {
                ...
                // Determinar el estilo del nombre en el chat
                ...
                } else if (d.usuario === 'Matías') {
                    estiloNombre = "color: var(--red);";
                }
    ...
    if(formAbismo) {
        formAbismo.addEventListener('submit', (e) => {
            e.preventDefault();
            if(currentUserName === "Ninja Anónimo") { alert("Debes Ingresar para compartir tu jugada."); window.location.hash = "#modal-login"; return; }
            const urlInput = document.getElementById('video-url').value.trim();
            let embedUrl = ""; let plataforma = "desconocida";
            if(urlInput.includes('youtube.com') || urlInput.includes('youtu.be')) { plataforma = "youtube"; const id = extraerIdLimpio(urlInput, 'youtube'); embedUrl = `https://www.youtube.com/embed/${id}`; } 
            else if(urlInput.includes('tiktok.com')) { plataforma = "tiktok"; const id = extraerIdLimpio(urlInput, 'tiktok'); if(id) { embedUrl = `https://www.tiktok.com/embed/v2/${id}`; } else { embedUrl = urlInput; } } 
            else { alert("Por ahora solo YouTube o TikTok."); return; }

            db.collection('abismo_videos').add({ usuario: currentUserName, url: embedUrl, urlCruda: urlInput, plataforma: plataforma, likes: 0, timestamp: firebase.firestore.FieldValue.serverTimestamp() })
            .then(() => { alert("¡Pergamino publicado en el Abismo!"); formAbismo.reset(); });
        });
    }
    ...
    if(formReporte) {
        formReporte.addEventListener('submit', (e) => {
            ...
            }).then(() => { alert("Reporte enviado exitosamente."); document.getElementById('modal-reporte').style.display = 'none'; window.location.hash = "#modal-llaves"; });
        });
    }
...
// ==========================================
// NUEVO: SISTEMA DE TIENDA Y ECONOMÍA
// ==========================================
...
function renderizarTienda() {
    ...
    CATALOGO_TIENDA.forEach(item => {
        ...
        if (currentUserName === "Ninja Anónimo") {
            ...
            botonHTML = `<button class="btn-primary" style="width: 100%;" onclick="comprarObjeto('${item.id}', ${item.precio})"><i class="fas fa-shopping-cart"></i> Comprar (${item.precio} R)</button>`;
        }
...
function comprarObjeto(id, precio) {
    ...
    if(confirm(`¿Deseas gastar ${precio} Ryos en este objeto?`)) {
        const nuevoSaldo = misRyos - precio;
...
function equiparObjeto(id, tipo) {
    // Copiamos el objeto actual para no borrar lo que ya tiene equipado de otro tipo
    const nuevoEquipamiento = { ...miEquipamiento };
    nuevoEquipamiento[tipo] = id;
    
    db.collection('ninjas').doc(currentUserId).update({
        equipado: nuevoEquipamiento
    }).then(() => {
        alert("Objeto equipado con éxito. ¡Ve a presumirlo!");
    });
}

function misionDiaria() {
    if(currentUserName === "Ninja Anónimo") return alert("Ingresa primero.");
    if(trabajando) return alert("Estás exhausto. Descansa unos segundos antes de volver a trabajar.");
    ...
    setTimeout(() => {
        db.collection('ninjas').doc(currentUserId).update({
            ...
            alert("Has barrido la arena de combate y ganaste 10 Ryos.");
        });
    }, 2000);
}
...
function abrirNotificaciones(e) {
    ...
    db.collection('notificaciones').where('para', '==', currentUserName).where('leida', '==', false).get().then(snap => {
        const batch = db.batch(); snap.forEach(doc => { batch.update(doc.ref, { leida: true }); }); batch.commit();
    });
}
...
function crearClan() {
    ...
    docRef.get().then(doc => {
        if(doc.exists) { alert("Ese nombre ya está en uso."); } 
        else { docRef.set({ nombre: nombre, lider: currentUserName, xp: 0, miembros: [currentUserName], timestamp: firebase.firestore.FieldValue.serverTimestamp() }).then(() => { db.collection('ninjas').doc(currentUserId).update({ clan: nombre }).then(() => { miClan = nombre; alert(`¡Has fundado el Escuadrón ${nombre}!`); abrirModalClan(); }); }); }
    });
}
function unirseClan() {
    ...
    const docRef = db.collection('clanes').doc(nombre);
    docRef.get().then(doc => {
        if(!doc.exists) { alert("Ese Escuadrón no existe."); } 
        else { docRef.update({ miembros: firebase.firestore.FieldValue.arrayUnion(currentUserName) }).then(() => { db.collection('ninjas').doc(currentUserId).update({ clan: nombre }).then(() => { miClan = nombre; alert(`¡Te uniste a ${nombre}!`); abrirModalClan(); }); }); }
    });
}
function abandonarClan() {
    if(!confirm("¿Deseas abandonar tu Escuadrón?")) return;
    db.collection('clanes').doc(miClan).update({ miembros: firebase.firestore.FieldValue.arrayRemove(currentUserName) }).then(() => { db.collection('ninjas').doc(currentUserId).update({ clan: "" }).then(() => { miClan = ""; alert("Has dejado tu escuadrón."); abrirModalClan(); }); });
}
...
function cargarTopClanes() {
    ...
    db.collection('clanes').orderBy('xp', 'desc').limit(10).onSnapshot(snap => {
        ...
        let puesto = 1;
        snap.forEach(doc => {
            const d = doc.data(); let color = "#333"; if(puesto === 1) color = "gold"; else if(puesto === 2) color = "silver"; else if(puesto === 3) color = "#cd7f32";
...
function cargarVideosAbismo() {
    ...
    db.collection('abismo_videos').orderBy('timestamp', 'desc').onSnapshot(snap => {
        lista.innerHTML = '';
        if(snap.empty) { lista.innerHTML = '<p style="color: #ccc; text-align: center; width: 100%;">El Abismo está en silencio.</p>'; return; }
        snap.forEach(doc => {
            const d = doc.data(); let reproductorHTML = "";
            if(d.url && d.url.includes('embed')) { reproductorHTML = `<iframe src="${d.url}" style="width: 100%; height: 350px; border: none; border-radius: 8px;" allowfullscreen></iframe>`; } 
            else { reproductorHTML = `<div style="height: 150px; display: flex; align-items: center; justify-content: center; background: #111; border-radius: 8px;"><a href="${d.urlCruda}" target="_blank" class="btn-secondary" style="text-decoration: none;"><i class="fas fa-external-link-alt"></i> Ver en TikTok</a></div>`; }

            // Buscar si el creador del video tiene borde (requiere consulta extra o guardar borde en el video. Por simplicidad, se muestra normal aquí)
            lista.innerHTML += `
                <div class="container-glass" style="padding: 15px; border-color: var(--blue) !important;">
                    ...
                    <div style="display: flex; justify-content: space-between; border-top: 1px solid #333; padding-top: 10px;">
                        <button style="background: none; border: none; color: #ccc; cursor: pointer; font-size: 1.1rem; transition: 0.2s;" onclick="darLikeVideo('${doc.id}', '${d.usuario}')" onmouseover="this.style.color='var(--red)'" onmouseout="this.style.color='#ccc'"><i class="fas fa-heart"></i> <span style="font-size: 0.9rem;">${d.likes || 0}</span></button>
                    </div>
                </div>
            `;
        });
...
function darLikeVideo(videoId, autorVideo) {
    if(currentUserName === "Ninja Anónimo") return alert("Identifícate primero.");
...
async function abrirPerfil(nick) {
    ...
    avatarImg.style = "width: 80px; height: 80px; border-radius: 50%; border: 2px solid #444; margin-bottom: 10px; transition: 0.3s;"; // Reset
    ...
    try {
        ...
        if(!snapshot.empty) {
            ...
            // Aplicar cosméticos de la tienda al perfil
            if(data.equipado) {
...
function filtrarTorneos(formato) {
    currentFilter = formato; const botones = document.querySelectorAll('.btn-filter'); botones.forEach(btn => btn.classList.remove('active')); event.target.classList.add('active'); cargarTorneosDesdeNube();
}
    ...
    db.collection('torneos').orderBy('timestamp', 'desc').onSnapshot(snap => {
        ...
        snap.forEach(doc => {
            ...
            if(currentFilter === 'todos' || data.formato === currentFilter) {
                ...
                if (data.estado === "iniciado" || data.estado === "finalizado") { btnTexto = "CERRADO"; btnColor = "gray"; } else if (yaInscrito) { btnTexto = "INSCRIPTO"; btnColor = "gray"; } else if (estaLleno) { btnTexto = "LLENO"; btnColor = "gray"; }
...
function unirseTorneo(torneoId, estado) {
    if(estado !== "abierto") return; if(currentUserName === "Ninja Anónimo") { alert("Debes ingresar con tu cuenta para unirte."); window.location.hash = "#modal-login"; return; }
...
function borrarTorneo(torneoId, torneoNombre) {
    if(confirm(`¿Estás seguro de ELIMINAR el torneo "${torneoNombre}"?`)) { db.collection('torneos').doc(torneoId).delete().then(() => { alert("Torneo borrado."); document.getElementById('contenedor-admin-partidos').innerHTML = ""; document.getElementById('btn-siguiente-ronda').style.display = "none"; }); }
}
async function generarLlaves(torneoId, torneoNombre) {
    if(!confirm(`¿Generar cruces para ${torneoNombre}?`)) return;
    const torneoRef = db.collection('torneos').doc(torneoId); const doc = await torneoRef.get(); const data = doc.data(); let jugadores = data.lista_inscriptos || []; if(jugadores.length < 2) { alert("Mínimo 2 ninjas."); return; }
...
async function generarSiguienteRonda(torneoId, rondaActual, partidos) {
    ...
    if(ganadores.length === 1) {
        ...
        // RECOMPENSA DE CAMPEÓN ACTUALIZADA (+100 XP, +500 RYOS)
        db.collection('ninjas').where('nick', '==', campeon).get().then(snap => {
            if(!snap.empty) {
                const docId = snap.docs[0].id; const userData = snap.docs[0].data();
                db.collection('ninjas').doc(docId).update({ xp: firebase.firestore.FieldValue.increment(100), ryos: firebase.firestore.FieldValue.increment(500) });
        ...
        enviarNotificacion(campeon, `¡ERES EL CAMPEÓN! Recibiste +100 XP y +500 Ryos.`);
        alert(`¡TORNEO FINALIZADO! Campeón: ${campeon}.`); window.location.hash = "#admin"; return;
    }
    const nuevosPartidos = [];
    for (let i = 0; i < ganadores.length; i += 2) { if (ganadores[i + 1]) { nuevosPartidos.push({ p1: ganadores[i], p2: ganadores[i + 1], ganador: "", ronda: rondaActual + 1 }); } else { nuevosPartidos.push({ p1: ganadores[i], p2: "BYE", ganador: ganadores[i], ronda: rondaActual + 1 }); } }
...
function verLlaves(torneoId, torneoNombre) {
    ...
    document.getElementById('llaves-titulo').innerText = `Llaves: ${torneoNombre}`; contenedor.innerHTML = '<p style="color: #888;">Leyendo pergaminos...</p>'; contenedorCampeon.innerHTML = ''; window.location.hash = "#modal-llaves";
    ...
    db.collection('torneos').doc(torneoId).collection('llaves').orderBy('ronda', 'asc').onSnapshot(snap => {
        ...
        snap.forEach(doc => {
            ...
            if (p.ganador === "" && p.p2 !== "BYE") {
                if (p.reporte_pendiente) { botonAccionHTML = `<div style="font-size: 0.7rem; color: gold; text-align: center; margin-top: 8px; border-top: 1px dashed #333; padding-top: 5px;"><i class="fas fa-clock"></i> Revisión pendiente del Kage...</div>`; } 
...
function abrirModalReporte(torneoId, partidoId, p1, p2) {
    document.getElementById('rep-torneo-id').value = torneoId; document.getElementById('rep-partido-id').value = partidoId;
    document.getElementById('rep-ganador').innerHTML = `<option value="" disabled selected>Selecciona al ganador...</option><option value="${p1}">${p1}</option><option value="${p2}">${p2}</option>`;
