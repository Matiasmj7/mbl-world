...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // ----------------------------------------------------
    // SISTEMA DE AUTENTICACIÓN Y CARGA DEL USUARIO
    // ----------------------------------------------------
    ...
    // ----------------------------------------------------
    // CONTROL DEL BYAKUGAN Y TICKER (EN VIVO)
    // ----------------------------------------------------
    db.collection('configuracion').doc('stream').onSnapshot(doc => { 
        if (doc.exists) { 
            ...
            document.getElementById('status-stream').innerText = `SEÑAL ACTUALIZADA: ${kageStreamPlat.toUpperCase()}`; 
    ...
    if(formConfigStream) { 
        formConfigStream.addEventListener('submit', (e) => { 
            ...
            }).then(() => alert("¡Visión Actualizada!")); 
        }); 
    }

    db.collection('configuracion').doc('ticker').onSnapshot(doc => {
        if (doc.exists) { 
            const texto = doc.data().mensaje; 
            const tickerEl = document.getElementById('ticker-contenido'); 
            if(tickerEl) { 
                tickerEl.innerHTML = `
                    <span class="ticker-item"><i class="fas fa-bullhorn"></i> MBL ARG INFO: <span style="color: var(--blue);">${texto}</span></span>
                    <span class="ticker-item"><i class="fas fa-fire"></i> ¡Prepárate para la batalla en la Arena!</span>
                `; 
            } 
        }
    });
    ...
    if(formTicker) { 
        formTicker.addEventListener('submit', (e) => { 
            e.preventDefault(); 
            db.collection('configuracion').doc('ticker').set({ 
                mensaje: document.getElementById('input-ticker').value, 
                timestamp: firebase.firestore.FieldValue.serverTimestamp() 
            }).then(() => alert("¡Alerta del Ticker enviada!")); 
        }); 
    }

    // ----------------------------------------------------
    // BANCO KAGE Y CREADOR DE EVENTOS
    // ----------------------------------------------------
    ...
    if(formT) { 
        formT.addEventListener('submit', (e) => { 
            ...
            db.collection('torneos').add({ 
                ...
                formT.reset(); 
                alert("¡Evento publicado con éxito en la Arena!"); 
            }); 
        }); 
    }

    // ----------------------------------------------------
    // LIBRO BINGO Y TABERNA
    // ----------------------------------------------------
    db.collection('ninjas').orderBy('xp', 'desc').limit(30).onSnapshot(snap => { 
        ...
        if(rankingContainer) { 
            ...
            if(snap.empty) { 
                rankingContainer.innerHTML = '<p style="color: #666; text-align: center;">El Libro Bingo está vacío.</p>'; 
            } 
            let puesto = 1; 
            snap.forEach(doc => { 
                const data = doc.data(); 
                let colorBorde = '#333'; 
                if(puesto === 1) colorBorde = 'gold'; 
                else if(puesto === 2) colorBorde = 'silver'; 
                else if(puesto === 3) colorBorde = '#cd7f32'; 
                ...
                rankingContainer.innerHTML += `
                    <div style="display: flex; justify-content: space-between; align-items: center; background: #000; padding: 10px 15px; border-radius: 5px; border-left: 3px solid ${colorBorde}; cursor: pointer; transition: 0.3s;" onclick="abrirPerfil('${data.nick}')">
    ...
    if(btnSendChat) { 
        btnSendChat.addEventListener('click', () => { 
            ...
            if(input.value.trim() && currentUserName !== "Ninja Anónimo") { 
                ...
                alert("Debes identificarte (Ingresar)."); 
            } 
        }); 
    }
    ...
    if(chatContainer) { 
        db.collection('taberna').orderBy('timestamp').onSnapshot(snap => { 
            ...
            snap.forEach(doc => { 
                ...
                chatContainer.innerHTML += `
                    <div style="margin-bottom: 8px; border-bottom: 1px solid #111; padding-bottom: 5px;">
                        <strong style="${estiloNombre} margin-right: 5px; cursor:pointer;" onclick="abrirPerfil('${d.usuario}')">${d.usuario}:</strong> 
                        <span style="word-break: break-all;">${d.texto}</span>
                    </div>`; 
            }); 
    ...
    // ----------------------------------------------------
    // EVENT LISTENERS FORMULARIOS VARIOS
    // ----------------------------------------------------
    ...
    if(formAbismo) { 
        formAbismo.addEventListener('submit', (e) => { 
            ...
            if(currentUserName === "Ninja Anónimo") { 
                alert("Debes Ingresar."); window.location.hash = "#modal-login"; return; 
            } 
            ...
            if(urlInput.includes('youtube.com') || urlInput.includes('youtu.be')) { 
                ...
                else { embedUrl = urlInput; } 
            } else { 
                alert("Por ahora solo YouTube o TikTok."); return; 
            } 
            ...
            db.collection('abismo_videos').add({ 
                ...
                alert("¡Pergamino publicado en el Abismo!"); 
                formAbismo.reset(); 
            }); 
        }); 
    }
...
function cargarHallOfFame() {
    ...
    // Solo carga ninjas que ya tengan el campo torneosGanados
    db.collection('ninjas').where('torneosGanados', '>', 0).orderBy('torneosGanados', 'desc').limit(3).onSnapshot(snap => {
        if(snap.empty) { 
            podio.innerHTML = '<p style="color: #666; width: 100%; text-align: center;">La historia recién comienza... ¿Quién será el primero en la cima?</p>'; 
            return; 
        }
...
function crearCartaPodio(ninja, rank) {
    ...
    if(ninja.equipado && ninja.equipado.borde) { 
        const b = CATALOGO_TIENDA.find(i => i.id === ninja.equipado.borde); 
        if(b) bordeEstilo = b.estilo; 
    }
    ...
    return `
        <div class="podium-spot rank-${rank}" style="position: relative; cursor:pointer;" onclick="abrirPerfil('${ninja.nick}')">
            ...
            <h4 style="color: white; margin-bottom: 5px; font-size: 0.9rem; word-break: break-all;">${ninja.nick}</h4>
            <span style="color: var(--green); font-size: 0.8rem; font-weight: bold;"><i class="fas fa-trophy"></i> ${ninja.torneosGanados} Copas</span>
            <span style="position: absolute; bottom: 5px; font-size: 2rem; opacity: 0.2; font-weight: 900;">${rank}</span>
        </div>
    `;
}

// ==========================================
// KAGE ADMIN: PURGAR TABERNA
// ==========================================
async function limpiarTaberna() { 
    if(!confirm("⚠️ ADVERTENCIA: Esto borrará TODOS los mensajes de la Taberna. ¿Proceder?")) return; 
    try { 
...
function renderizarTienda() { 
    ...
    CATALOGO_TIENDA.forEach(item => { 
        ...
        if (currentUserName === "Ninja Anónimo") { 
            ...
            let textoPrecio = auth.currentUser?.email === ADMIN_EMAIL ? "Obtener Gratis" : `Comprar (${item.precio} R)`; 
...
function comprarObjeto(id, precioBase) { 
    ...
    let mensajeConfirmacion = `¿Gastar ${precioBase} Ryos?`; 
    
    if (auth.currentUser && auth.currentUser.email === ADMIN_EMAIL) { 
        costoFinal = 0; mensajeConfirmacion = `¿Obtener GRATIS (Poder Kage)?`; 
    } else if (misRyos < precioBase) { 
        alert("Ryos insuficientes."); return; 
    } 
...
function equiparObjeto(id, tipo) { 
    ...
    db.collection('ninjas').doc(currentUserId).update({ equipado: nuevoEquipamiento }).then(() => { alert("¡Equipado con éxito!"); }); 
}
    ...
    db.collection('ninjas').doc(currentUserId).get().then(doc => { 
        if (doc.exists) { 
            ...
            if (fechaUltimoTrabajo === hoy) { 
                if (countTrabajosHoy >= 3) { alert("Has alcanzado el límite diario (30 Ryos)."); return; } 
                countTrabajosHoy++; 
            } else { 
            ...
            setTimeout(() => { 
                db.collection('ninjas').doc(currentUserId).update({ 
                    ...
                    alert(`Ganaste 10 Ryos. (${countTrabajosHoy}/3 hoy)`); 
                }); 
            }, 1500); 
        } 
    }); 
...
function escucharNotificaciones() { 
    ...
    db.collection('notificaciones').where('para', '==', currentUserName).orderBy('timestamp', 'desc').onSnapshot(snap => { 
        ...
        if(snap.empty) { 
            contenedorHTML.innerHTML = '<p style="color:#888; text-align:center;">No hay mensajes.</p>'; 
        ...
        if(noLeidas > 0) { badge.innerText = noLeidas; badge.style.display = 'inline-block'; } 
...
async function abrirPerfil(nick) { 
    ...
    document.getElementById('perfil-nick').style = "color: white; margin-bottom: 5px; transition: 0.3s;"; 
    ...
    if (nick === currentUserName && currentUserName !== "Ninja Anónimo") { btnEditar.style.display = 'block'; } 
    ...
    try { 
        ...
        if(!snapshot.empty) { 
            ...
            if(data.redSocial && data.redSocial !== "") { 
                document.getElementById('perfil-redes-container').innerHTML = `<a href="${data.redSocial}" target="_blank" style="color: var(--blue); text-decoration: none; font-size: 0.9rem; border: 1px solid var(--blue); padding: 5px 10px; border-radius: 5px; display: inline-block; margin-top: 5px;"><i class="fas fa-link"></i> Visitar Red Social</a>`; 
            } 
            ...
            if(data.equipado) { 
                if(data.equipado.borde) { 
                    const b = CATALOGO_TIENDA.find(i => i.id === data.equipado.borde); 
                    if(b) avatarImg.style = `width: 100px; height: 100px; object-fit: cover; border-radius: 50%; margin-bottom: 10px; transition: 0.3s; background: #111; ${b.estilo}`; 
                } 
                if(data.equipado.colorChat) { 
                    const c = CATALOGO_TIENDA.find(i => i.id === data.equipado.colorChat); 
                    if(c) document.getElementById('perfil-nick').style = `margin-bottom: 5px; transition: 0.3s; font-weight: bold; ${c.estilo}`; 
                } 
                if(data.equipado.pin) { 
                    const p = CATALOGO_TIENDA.find(i => i.id === data.equipado.pin); 
                    if(p) document.getElementById('perfil-pin-container').innerHTML = p.icon; 
                } 
            } 
        } else { 
            document.getElementById('perfil-rango').innerText = 'Sin Rango'; 
            document.getElementById('perfil-xp').innerText = `0 XP`; 
            document.getElementById('perfil-campeonatos').innerText = 0;
        } 
        ...
        if(doc.exists) { alert("Ese nombre ya está en uso."); } 
            ...
            docRef.set({ 
                ...
                db.collection('ninjas').doc(currentUserId).update({ clan: nombre }).then(() => { 
                    miClan = nombre; alert(`¡Escuadrón ${nombre} creado!`); abrirModalClan(); 
                }); 
            }); 
        } 
    }); 
...
function abandonarClan() { 
    ...
    db.collection('clanes').doc(miClan).update({ miembros: firebase.firestore.FieldValue.arrayRemove(currentUserName) }).then(() => { 
        db.collection('ninjas').doc(currentUserId).update({ clan: "" }).then(() => { 
            miClan = ""; alert("Has dejado tu escuadrón."); abrirModalClan(); 
        }); 
    }); 
}
...
function cargarTopClanes() { 
    ...
    db.collection('clanes').orderBy('xp', 'desc').limit(10).onSnapshot(snap => { 
        lista.innerHTML = ""; 
        if(snap.empty) { lista.innerHTML = "<p style='color:#666; text-align:center;'>Aún no hay escuadrones.</p>"; return; } 
        let puesto = 1; 
        snap.forEach(doc => { 
            const d = doc.data(); 
        ...
        snap.forEach(doc => { 
            ...
            lista.innerHTML += `
                <div class="container-glass" style="padding: 15px; border-color: var(--blue) !important;">
                    <div style="display: flex; align-items: center; gap: 10px; margin-bottom: 10px; cursor: pointer; transition: 0.3s;" onclick="abrirPerfil('${d.usuario}')">
                    ...
                    <div style="display: flex; justify-content: space-between; border-top: 1px solid #333; padding-top: 10px;">
                        <button style="background: none; border: none; color: #ccc; cursor: pointer; font-size: 1.1rem; transition: 0.2s;" onclick="darLikeVideo('${doc.id}', '${d.usuario}')"><i class="fas fa-heart"></i> <span style="font-size: 0.9rem;">${d.likes || 0}</span></button>
                    </div>
                </div>`; 
        }); 
    }); 
...
function cargarTorneosDesdeNube() {
    ...
    db.collection('torneos').orderBy('timestamp', 'desc').onSnapshot(snap => {
        ...
        let hayTorneosVisibles = false; let hayLigasVisibles = false;
...
function unirseTorneo(torneoId, estado) { 
    ...
    if(currentUserName === "Ninja Anónimo") { alert("Debes ingresar."); window.location.hash = "#modal-login"; return; } 
    ...
    torneoRef.get().then(doc => { 
        if (doc.exists) { 
            const data = doc.data(); 
            const inscritos = data.lista_inscriptos ? data.lista_inscriptos.length : 0; 
            if (inscritos >= data.cuposTotales) { alert("Evento lleno."); } 
            else { 
    ...
    db.collection('torneos').orderBy('timestamp', 'desc').onSnapshot(snap => { 
        ...
        snap.forEach(doc => { 
            ...
            if(d.estado === 'iniciado') { 
                botonesHTML = `<button class="btn-secondary" style="padding: 8px 15px; font-size: 0.8rem; margin-right: 5px;" onclick="abrirAdminPartidos('${id}', '${d.nombre}')">ADMINISTRAR</button>
            ...
            lista.innerHTML += `
                <div style="display: flex; justify-content: space-between; align-items: center; background: #000; padding: 15px; border-radius: 8px; border: 1px solid #222; flex-wrap: wrap; gap: 10px;">
                    <div style="flex: 1; min-width: 150px;"><strong>${etiquetaTipo} ${d.nombre}</strong> <br> <span style="font-size:0.7rem; color:#888;">${d.lista_inscriptos?.length || 0} inscritos</span></div>
                    <div style="display: flex; flex-direction: column; align-items: flex-end;">${botonesHTML}</div>
                    <div style="width: 100%; margin-top: 5px; border-top: 1px dashed #333; padding-top: 10px;"><button class="btn-secondary" style="background: transparent; color: var(--red); border: 1px solid var(--red); padding: 6px; font-size: 0.7rem; width: 100%;" onclick="borrarTorneo('${id}', '${d.nombre}')"><i class="fas fa-trash"></i> Eliminar Evento</button></div>
                </div>`; 
        }); 
    });
}
...
function borrarTorneo(torneoId, torneoNombre) { 
    if(confirm(`¿ELIMINAR "${torneoNombre}"?`)) { 
...
async function generarLlaves(torneoId, torneoNombre) { 
    ...
    jugadores.forEach(j => { enviarNotificacion(j, `¡Los cruces están listos en "${torneoNombre}"!`); }); 
            ...
            if(p.ganador !== "") { 
                ...
                contenedor.innerHTML += `
                    <div style="background: #111; padding: 10px; margin-bottom: 10px; border-radius: 5px; border: 1px solid gold;">
                        ...
                        <div style="background: rgba(255, 215, 0, 0.1); padding: 8px; border-radius: 5px; margin-bottom: 10px; font-size: 0.9rem;">
                            ...
                            <span style="color: var(--green); font-weight: bold; font-size: 1.1rem;">${p.reporte_pendiente.ganadorPropuesto}</span><br>${pruebaHtml}
                        </div>
                ...
                contenedor.innerHTML += `
                    <div style="background: #111; padding: 10px; margin-bottom: 10px; border-radius: 5px; border: 1px solid var(--blue);">
                        ...
                        <p style="text-align: center; font-size: 0.7rem; color: #888; margin-bottom: 5px;">Esperando reporte...</p>
                        <div style="display: flex; gap: 5px;">
...
function setGanador(torneoId, partidoId, ganador) { 
    if(confirm(`¿Confirma que ${ganador} avanza?`)) { 
        db.collection('torneos').doc(torneoId).collection('llaves').doc(partidoId).update({ ganador: ganador, reporte_pendiente: firebase.firestore.FieldValue.delete() }).then(() => { 
            enviarNotificacion(ganador, "El Kage aprobó el reporte. ¡Avanzas de ronda!"); 
        }); 
    } 
}

// ==========================================
// FINALIZAR EVENTO Y PREMIAR (ALIMENTA SALÓN DE FAMA)
// ==========================================
async function generarSiguienteRonda(torneoId, rondaActual, partidos) {
    ...
    if(ganadores.length === 1) {
        ...
        // PREMIAR AL CAMPEÓN: +100 XP, +500 RYOS Y +1 TORNEO GANADO
        db.collection('ninjas').where('nick', '==', campeon).get().then(snap => {
            if(!snap.empty) { 
                const docId = snap.docs[0].id; 
                const userData = snap.docs[0].data(); 
                
                db.collection('ninjas').doc(docId).update({ 
                    xp: firebase.firestore.FieldValue.increment(100), 
                    ryos: firebase.firestore.FieldValue.increment(500),
...
function verLlaves(torneoId, torneoNombre) { 
    ...
    db.collection('torneos').doc(torneoId).get().then(doc => { 
        if(doc.exists && doc.data().estado === 'finalizado') { 
            contenedorCampeon.innerHTML = `
                <div style="background: rgba(255, 215, 0, 0.1); border: 1px solid gold; padding: 15px; text-align: center; border-radius: 8px; margin-bottom: 20px;">
                    <h4 style="color: gold; margin-bottom: 5px;"><i class="fas fa-crown"></i> GRAN CAMPEÓN</h4>
                    <strong style="font-size: 1.5rem; cursor:pointer;" onclick="abrirPerfil('${doc.data().campeon}')">${doc.data().campeon}</strong>
                </div>`; 
        } 
    }); 
