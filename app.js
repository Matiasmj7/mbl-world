...
function cargarTopComunidades() {
    ...
    db.collection('comunidades').onSnapshot(snap => {
        ...
        if(comunidades.length === 0) { 
            lista.innerHTML = "<p style='color:#666; text-align:center;'>Ninguna alianza forjada aún.</p>"; 
            return; 
        }
        ...
        comunidades.slice(0, 5).forEach((d, index) => {
            ...
            lista.innerHTML += `
                <div style="display:flex; justify-content:space-between; align-items:center; background:rgba(0,0,0,0.5); padding:12px; border-radius:5px; border-left:3px solid ${color}; margin-bottom: 8px;">
                    <div>
                        <strong style="color: white; font-size: 1.1rem;">${index + 1}. ${d.nombre}</strong> <br> 
...
function equiparObjeto(id, tipo) {
    ...
    }).then(() => alert("¡Objeto equipado con éxito!"));
}
...
function generarTarjetaEventoHTML(data, id, esLiga) {
    ...
    let btnTexto = "UNIRSE A LA BATALLA"; 
    let btnColor = "var(--blue)";
    
    if (data.estado === "iniciado" || data.estado === "finalizado") { 
    ...
    torneoRef.get().then(doc => {
        if (doc.exists) {
            ...
            if (inscritos >= data.cuposTotales) { 
                ...
                }).then(() => alert("¡Inscripto exitosamente!"));
            }
        }
    });
}
...
function cargarVideosAbismo() {
    ...
    db.collection('abismo_videos').orderBy('timestamp', 'desc').onSnapshot(snap => {
        lista.innerHTML = '';
        if(snap.empty) { 
            lista.innerHTML = '<p style="color: #ccc; text-align: center; width: 100%;">El Abismo está en silencio. Sube una jugada.</p>'; 
            return; 
        }
        ...
        snap.forEach(doc => {
            ...
            if(d.plataforma === 'youtube') {
                const videoId = d.url.split('embed/')[1];
            ...
            if (d.comentarios && d.comentarios.length > 0) {
                comentariosHTML = d.comentarios.slice(-3).map(c => `
                    <div class="comentario-box">
                        <strong style="color:var(--blue);">${c.usuario}:</strong> 
                        <span style="color:#ddd;">${c.texto}</span>
                    </div>`
                ).join('');
            }
            ...
            lista.innerHTML += `
                <div class="container-glass" style="padding: 15px; border-color: var(--blue) !important;">
                    ...
                    <div class="video-preview-card" id="cont-${id}" onclick="activarVideo('${id}', '${d.url}')">
                        ${btnDelete}
                        <img src="${thumb}" class="thumbnail-img" alt="Miniatura de video">
                    ...
                    <div style="display: flex; justify-content: space-between; border-top: 1px solid #333; padding-top: 10px; margin-top: 10px;">
                        <button style="background: none; border: none; color: #ccc; cursor: pointer; font-size: 1.1rem;" onclick="darLikeVideo('${id}', '${d.usuario}')">
                            <i class="fas fa-heart" style="color: var(--red);"></i> 
                            <span style="font-size: 0.9rem; color:white;">${d.likes || 0}</span>
                        </button>
                    </div>
                    ...
                    <div style="margin-top: 10px; border-top: 1px solid #222; padding-top: 10px;">
                        ...
                        <form class="comentario-input-group" onsubmit="comentarVideo(event, '${id}', '${d.usuario}')">
                            <input type="text" id="coment-${id}" class="comentario-input" placeholder="Agregar un comentario..." required>
...
function activarVideo(docId, url) {
    const contenedor = document.getElementById(`cont-${docId}`);
    contenedor.innerHTML = `<iframe src="${url}?autoplay=1" style="width: 100%; aspect-ratio: 16/9; border: none;" allow="autoplay; fullscreen" allowfullscreen></iframe>`;
...
function cargarTopClanes() {
    ...
    db.collection('clanes').orderBy('xp', 'desc').limit(5).onSnapshot(snap => {
        lista.innerHTML = "";
        if(snap.empty) { 
            lista.innerHTML = "<p style='color:#666; text-align:center;'>Aún no hay escuadrones formados.</p>"; 
            return; 
        }
        ...
        snap.forEach(doc => {
            const d = doc.data();
            let color = "#333"; 
            if(puesto === 1) color = "gold"; 
            else if(puesto === 2) color = "silver"; 
            else if(puesto === 3) color = "#cd7f32";
            
            lista.innerHTML += `
                <div style="display:flex; justify-content:space-between; align-items:center; background:rgba(0,0,0,0.5); padding:12px; border-radius:5px; border-left:3px solid ${color}; margin-bottom: 8px;">
                    <div>
                        <strong style="color: white; font-size: 1.1rem;">${puesto}. ${d.nombre}</strong> <br> 
                        <span style="font-size:0.75rem; color:#888;"><i class="fas fa-users"></i> ${d.miembros.length} miembros</span>
                    </div>
...
function cargarAnunciosGremio() {
    ...
    db.collection('anuncios_gremio').orderBy('timestamp', 'desc').limit(15).onSnapshot(snap => {
        lista.innerHTML = "";
        if(snap.empty) { 
            lista.innerHTML = "<p style='color:#888; text-align:center;'>El tablón está vacío.</p>"; 
            return; 
        }
...
function cargarTopIndividualBingo() {
    ...
    db.collection('ninjas').orderBy('xp', 'desc').limit(15).onSnapshot(snap => {
        ...
        snap.forEach(doc => {
            ...
            if(d.equipado && d.equipado.colorChat) { 
                const itemColor = CATALOGO_TIENDA.find(i => i.id === d.equipado.colorChat); 
            ...
            lista.innerHTML += `
                <div style="display: flex; justify-content: space-between; align-items: center; background: rgba(0,0,0,0.5); padding: 10px 15px; border-radius: 5px; border-left: 3px solid ${colorBorde}; cursor: pointer; transition: 0.3s;" onclick="abrirPerfil('${d.nick}')">
                    <div>
                        <strong>${puesto}. <span style="${colorNombre.includes('color:') ? colorNombre : 'color:'+colorNombre}">${d.nick}</span></strong>
                    </div>
...
async function abrirPerfil(nick) {
    ...
    try {
        ...
        if(!snapshot.empty) {
            ...
            if(data.fotoPerfil && data.fotoPerfil !== "") { 
                avatarImg.src = data.fotoPerfil; 
            }
            if(data.bio && data.bio !== "") { 
                document.getElementById('perfil-bio').innerText = `"${data.bio}"`; 
            }
            ...
            if(data.comunidad && data.comunidad !== "") { 
                document.getElementById('perfil-comunidad').innerHTML = `<i class="fas fa-handshake"></i> Alianza: ${data.comunidad}`; 
            }
...
function crearClan() {
    ...
    db.collection('clanes').doc(nombre).get().then(doc => {
        if(doc.exists) { 
            alert("Ese Escuadrón ya existe."); 
        } else { 
            db.collection('clanes').doc(nombre).set({ 
                nombre: nombre, 
                lider: currentUserName, 
                miembros: [currentUserName], 
                xp: 0, 
                ...
                db.collection('ninjas').doc(currentUserId).update({ clan: nombre }); 
                alert("¡Escuadrón formado!"); 
            }); 
        }
    });
}
...
function unirseClan() {
    ...
    db.collection('clanes').doc(nombre).get().then(doc => {
        if(!doc.exists) { 
            alert("El Escuadrón no existe."); 
        } else { 
            db.collection('clanes').doc(nombre).update({ 
                miembros: firebase.firestore.FieldValue.arrayUnion(currentUserName) 
            }).then(() => { 
                db.collection('ninjas').doc(currentUserId).update({ clan: nombre }); 
    ...
    if(confirm("¿Abandonar tu Escuadrón?")) {
        db.collection('clanes').doc(miClan).get().then(doc => {
            if(doc.exists) {
                ...
                if(data.miembros.length === 1) { 
                    db.collection('clanes').doc(miClan).delete(); 
                } else { 
...
function escucharTicker() {
    db.collection('configuracion').doc('ticker').onSnapshot(doc => {
        if(doc.exists) { 
            document.getElementById('ticker-contenido').innerHTML = `
                <span class="ticker-item"><i class="fas fa-bullhorn"></i> ALERTA: <span style="color: var(--blue);">${doc.data().mensaje}</span></span> 
                <span class="ticker-item"><i class="fas fa-fire"></i> ¡La Arena está lista!</span>`; 
        }
    });
}
    ...
    if(formTorneo) {
        formTorneo.addEventListener('submit', (e) => {
            ...
            db.collection('torneos').add({
                ...
                formTorneo.reset(); 
                alert("¡Evento publicado en la Arena!"); 
            });
        });
    }
...
function banearUsuario() {
    const nick = document.getElementById('gestion-nick').value.trim(); 
    if(!nick) return;
            ...
            lista.innerHTML += `
                <div style="display: flex; justify-content: space-between; align-items: center; background: #000; padding: 15px; border-radius: 8px; border: 1px solid #222; flex-wrap: wrap; gap: 10px;">
                    <div style="flex: 1; min-width: 150px;">
                        <strong>${etiquetaTipo} ${d.nombre}</strong> <br> 
                        <span style="font-size:0.7rem; color:#888;">${d.lista_inscriptos?.length || 0} inscritos</span>
                    </div>
                    ...
                    <div style="width: 100%; margin-top: 5px; border-top: 1px dashed #333; padding-top: 10px;">
                        <button class="btn-secondary" style="background: transparent; color: var(--red); border: 1px solid var(--red); padding: 6px; font-size: 0.7rem; width: 100%;" onclick="borrarTorneo('${id}', '${d.nombre}')"><i class="fas fa-trash"></i> Eliminar Evento</button>
                    </div>
                </div>`; 
        }); 
    });
...
async function generarLlaves(torneoId, torneoNombre) { 
    ...
    for (let i = 0; i < jugadores.length; i += 2) { 
        if (jugadores[i + 1]) { 
            partidos.push({ p1: jugadores[i], p2: jugadores[i + 1], ganador: "", ronda: 1 }); 
        } else { 
            partidos.push({ p1: jugadores[i], p2: "BYE", ganador: jugadores[i], ronda: 1 }); 
        } 
    } 
...
function abrirAdminPartidos(torneoId, torneoNombre, creador) { 
    ...
    db.collection('torneos').doc(torneoId).collection('llaves').orderBy('ronda', 'desc').onSnapshot(snap => { 
        ...
        if(snap.empty) { 
            contenedor.innerHTML = "<p>Las llaves no se han generado aún.</p>"; 
            ...
            if(p.ganador !== "") { 
                ...
                contenedor.innerHTML += `
                    <div style="background: #111; padding: 10px; margin-bottom: 10px; border-radius: 5px; border: 1px solid var(--blue);">
                        ...
                        <div style="display: flex; gap: 5px;">
                            <button class="btn-secondary" style="flex: 1; padding: 5px; font-size:0.8rem;" onclick="setGanador('${torneoId}', '${p.id}', '${p.p1}')">Forzar: ${p.p1}</button>
                            <button class="btn-secondary" style="flex: 1; padding: 5px; font-size:0.8rem;" onclick="setGanador('${torneoId}', '${p.id}', '${p.p2}')">Forzar: ${p.p2}</button>
                        </div>
                    </div>`; 
            } 
        }); 
...
function inscribirJugadorManual() {
    ...
    db.collection('torneos').doc(tId).update({ 
        lista_inscriptos: firebase.firestore.FieldValue.arrayUnion(nick) 
    }).then(() => { 
        alert(`${nick} añadido al torneo.`); 
...
async function generarSiguienteRonda(torneoId, rondaActual, partidos) {
    ...
    ganadores.forEach(g => { 
        enviarNotificacion(g, `La Ronda ${rondaActual + 1} ha sido generada. ¡Prepárate!`); 
    }); 
...
function escucharNotificaciones() {
    ...
    db.collection('notificaciones').where('para', '==', currentUserName).orderBy('timestamp', 'desc').onSnapshot(snap => {
        ...
        if(snap.empty) { 
            contenedorHTML.innerHTML = '<p style="color:#888; text-align:center;">No hay avisos.</p>'; 
